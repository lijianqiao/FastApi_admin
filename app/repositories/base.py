...
@FileName: base.py
@DateTime: 2025/06/03
@Docs: ORM基类和Repository基类 - 使用 advanced-alchemy 的基类，采用最小最优设计 (优化版)
"""
...
class AbstractBaseRepository(SQLAlchemyAsyncRepository[ModelT], Generic[ModelT, IdType], ABC):
    ...
    def __init__(self, session: AsyncSession, **kwargs: Any) -> None:
        ...
        if not hasattr(self, "model_type") or self.model_type is None:
            logger.error(f"存储库 {self.__class__.__name__} 没有定义 'model_type' 属性，无法进行数据操作。")
    ...
    async def get_by_id(
        ...
        return await self.get_one_or_none(*processed_filters, auto_expunge=auto_expunge)
    ...
    async def get_all_records(
        ...
        *filters: FilterTypes,
        ...
        获取所有符合条件的记录。
        """
        processed_filters = self._apply_soft_delete_filter(list(filters), include_deleted)
        return await self.list(*processed_filters, auto_expunge=auto_expunge)
    ...
    async def update_record(self, item_id: IdType, data: ModelT | dict[str, Any]) -> ModelT | None:
        """
        更新指定ID的记录。
        注意：此方法默认不获取已软删除的记录进行更新。
        如果需要更新已软删除的记录，请先通过 get_by_id(..., include_deleted=True) 获取。
        """
        db_obj = await self.get_by_id(item_id, include_deleted=False)  # 默认不操作已软删除的
        if not db_obj:
            return None
    ...
    async def delete_record(self, item_id: IdType, hard_delete: bool = False) -> ModelT | None:
        ...
        删除指定ID的记录。
        """
        # 获取对象时需要考虑 include_deleted=True，以便能够对已软删除的对象执行硬删除
    ...
    async def count_records(self, *filters: FilterTypes, include_deleted: bool = False) -> int:
        ...
        # 使用 advanced_alchemy 内置的 count 方法，它能正确处理过滤器
